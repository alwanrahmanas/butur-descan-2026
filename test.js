        ...
        const SPREADSHEET_ID = "1FZZpyF6lG6cvPUu-i8gn9WcOMhExN_OG";
        const SHEET_GIDS = {
            keluarga: "1804314654",
            lapangan: "1446044024",
        ...
        function renderDashboardKeluarga() {
            ...
            const listrikPLN = klg.filter(k => String(k.penerangan_rumah || '').includes("PLN")).length;
            const jamban = klg.filter(k => String(k.fasilitas_bab || '').includes("keluarga sendiri")).length;
            const pkh = klg.filter(k => k.penerima_pkh === 'Ya').length;
            const bpjsPbi = klg.filter(k => k.penerima_bpjs_pbi === 'Ya').length;
            ...
            // Charts
            const sTinggal = countBy(klg, 'tempat_tinggal');
            makeChart('chartTinggal', 'doughnut', Object.keys(sTinggal), Object.values(sTinggal));

            // Bantuan
            const programs = [
                { key: 'penerima_blt_desa', label: 'BLT Desa' },
                { key: 'penerima_pkh', label: 'PKH' },
                { key: 'penerima_bpjs_pbi', label: 'BPJS PBI' },
                { key: 'penerima_bantuan_pangan', label: 'Bantuan Pangan' },
                { key: 'penerima_bnpt_sembako', label: 'BNPT/Sembako' },
                { key: 'penerima_pip', label: 'PIP' },
                { key: 'penerima_mbg', label: 'MBG' }
            ];
        ...
        function showPhoto(kk) {
            ...
            const url = `https://drive.google.com/uc?export=view&id=${data.file_id}`;
            document.getElementById('photoPopupImage').src = url;
        ...
        function closePhotoPopup() {
            ...
            document.getElementById('photoPopupImage').src = '';
        }
                ...
                complete: function(results) {
                    if (results.data && results.data.length > 0) {
                        ...
                        if (row && row.link) {
                            ...
                            setTimeout(() => {
                                document.getElementById('photoPopupImage').src = `https://drive.google.com/uc?export=view&id=${fileId}`;
                                document.getElementById('photoPopupOverlay').classList.add('active');
        ...
        window.exportAgregat = function (type) {
            ...
            if (type === 'keluarga' || type === 'gabungan') {
                ...
                const rows = klg.map((k, i) => [
                    ...
                    k.tempat_tinggal, k.jenis_atap || '-', k.jenis_dinding || '-', k.jenis_lantai || '-',
                    k.energi_memasak || '-', k.penerangan_rumah || '-', k.sumber_air_minum || '-',
                    k.fasilitas_bab || '-', k.penerima_pkh || '-', k.penerima_bpjs_pbi || '-', k.penerima_blt_desa || '-'
                ]);
